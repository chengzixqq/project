...
function calculateSchedule() {
  ...
  const input = buildSchedulerInputFromState(state, { effectiveCd });
  const result = generateSchedule(input);
  state.events = result.events || [];
  state.scheduleStats = result.stats || null;
...
function init() {
  $('filterNoCast')?.addEventListener('change', () => {
    if (state.events?.length) renderResults(state.events, state.scheduleStats);
  });
  ...
  document.querySelectorAll('input[name="sched"]').forEach((r) => r.addEventListener('change', () => {
    state.schedMode = getSchedMode();
  }));
