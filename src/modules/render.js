...
export function renderResults(eventsAll) {
  ...
  events.forEach((e) => {
    ...
    if (e.type === 'skip') tr.classList.add('row-skip');
    if (e.type === 'vacuum') {
      tr.classList.add('row-vacuum');
      if (e.death === true) tr.classList.add('row-death');
    }
    const actionText = e.type === 'skill'
      ? `<b>${e.name}</b>`
      : e.type === 'skip'
        ? `<b>跳过：${e.name}</b>`
        : `<b>${e.death === true ? '死亡真空（单段真空 ≥ 阈值）' : '真空（无可施放技能）'}</b>${e.note ? `<div class="muted">${e.note}</div>` : ''}`;
